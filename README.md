...
- **Day 6**：借阅闭环与查询下推 —— 改写 `borrow`（减库存 + 写借阅记录，把判断与修改压进一条 SQL 解决超卖）、新增归还接口、`relationship` 的实战用法，并把分页、排序、筛选从 Python 层下推给数据库。
- **Day 7**：Alembic 迁移入门（初始化、生成第一个迁移文件，与 `create_all` 的关系）。
...
> 4. **把原本一天的「表关系与借阅闭环」拆成 Day 5 / Day 6**，Day 5 只立数据模型（模型 + 外键 + SQLite 外键开关），Day 6 才做接口和并发。原因是合在一起时，一节课里同时要理解"数据怎么关联"和"并发怎么正确"两件互不相关的事。腾出的位置让「查询的下推」并入了 Day 6 —— 它和"查借阅记录"是同一个场景，本来就要分页。
