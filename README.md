...
- **Day 0**：环境准备 —— 用 Docker 跑一个 Redis 容器。**只学第 12 周需要的部分**：拉镜像、`docker run` 起容器、端口映射、查看日志、启动 / 停止 / 删除容器、验证能连上。**不讲** Dockerfile、多阶段构建、Compose 编排与镜像瘦身 —— 那些仍按原计划留在第 15 周。
...
> **计划调整说明（第 12 周）**：Redis 官方不提供 Windows 版本，第 12 周必须先有可运行的 Redis 服务。
> 原方案是把第 15 周的 Docker 内容整体提前——但那会打乱第 15 周（Nginx、Gunicorn 进程模型、镜像瘦身）的结构。
> 最终采用的方案是**在第 12 周前加一个 Day 0，只教"跑起一个 Redis 容器"这一个动作**，范围锁死在"本周用得到"的边界内，第 15 周不动。
