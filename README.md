...
- **Day 3**：测试隔离 —— 用 `dependency_overrides` 给测试一套独立数据库，让第 7 周写的 82 个测试重新跑起来。
- **Day 4**：表关系与外键 —— 新增借阅记录表，一对多建模与 `relationship`。
- **Day 5**：借阅闭环 —— 借书 / 还书接口，以及库存扣减的并发正确性（把判断与修改压进一条 SQL）。
...
> 实际执行时做了两处调整：
> 1. **把「测试隔离」提前到 Day 3。** 原因是 Day 1 之后 82 个测试全部失效（它们靠清内存字典来隔离数据），在没有任何回归网的状态下新增表和外键，一旦出错将无法判断问题来自新代码还是既有的断裂。
> 2. **原 Day 4「迁移的升级、回滚与版本管理」并入 Alembic 当天**，腾出的时间用于新增「借阅闭环」一天，用来补上项目里 `borrow` 只减库存、不记录借阅人的真实缺口。
