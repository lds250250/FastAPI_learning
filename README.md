...
- **Day 2**：分层架构设计与目录重构（路由层、业务层、数据层拆分与 `src-layout` 规范）。
- **Day 3**：`APIRouter` 路由拆分与模块化组装（前缀、标签分组与多模块注册）。
- **Day 4**：中间件基础（CORS 跨域、请求日志记录与全局异常处理器）。
- **Day 5**：业务逻辑抽离与服务层封装（将路由函数中的处理逻辑迁移至独立 Service 模块）。
- **Day 6**：第四周综合实战（上）—— 按分层架构重构现有用户模块，完成路由拆分与服务层迁移。
- **Day 7**：第四周综合实战（下）—— 验证所有接口兼容性，补充 `.env.example`、更新 README 与架构笔记。
