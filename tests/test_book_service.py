...
async def service(db_factory):
    async with db_factory() as session:
        yield BookService(BookRepository(session))
...
@pytest.mark.anyio
async def test_register_borrow_is_right(service):
    await service.create(make_book())

    result = await service.borrow("9787115428028")

    assert result["stock"] == 4


@pytest.mark.anyio
async def test_register_borrow_zero_raises(service):
    await service.create(make_book(stock=0))

    with pytest.raises(OutOfStock):
        await service.borrow("9787115428028")
