...
def test_create_book(auth_client):
    response = auth_client.post("/books/", json=make_book_payload())
...
def test_create_duplicate_isbn_returns_409(auth_client):
    assert auth_client.post("/books/", json=make_book_payload()).status_code == 201
    response = auth_client.post("/books/", json=make_book_payload())
...
def test_delete_book_requires_admin(auth_client):
    auth_client.post("/books/", json=make_book_payload())
